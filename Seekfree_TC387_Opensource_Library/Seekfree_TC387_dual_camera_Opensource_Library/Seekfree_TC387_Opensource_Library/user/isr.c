...
// 简单点说实际上进入中断后 TC 系列的硬件自动调用了 interrupt_global_disable();
...
// 由于摄像头 pclk 引脚默认占用了 1 通道，用于触发 DMA，因此这里不再定义中断函数
IFX_INTERRUPT(exti_ch1_ch5_isr, 0, EXTI_CH1_CH5_INT_PRIO)
{
    ...
    if(exti_flag_get(ERU_CH1_REQ10_P14_3))          // 通道1中断
    {
        ...
        // 注意: 该外部中断为摄像头 PCLK 若已使用摄像头 请不要再添加其他内容
    }
...
IFX_INTERRUPT(uart0_rx_isr, UART0_INT_VECTAB_NUM, UART0_RX_INT_PRIO)
{
    ...
    debug_interrupr_handler();                      // 调用 debug 串口接收处理函数
...
IFX_INTERRUPT(uart3_rx_isr, UART3_INT_VECTAB_NUM, UART3_RX_INT_PRIO)
{
    ...
    gnss_uart_callback();                            // GNSS 串口回调函数

#if MOTOR_USE_BRUSHLESS_UART
    motor_uart_callback();                           // 双驱无刷电机串口回调函数
#endif
}
