...
// INT_SERVICE：宏定义决定中断由谁处理，也称为服务提供者（在TC387中，中断被叫做服务），
//              可设置范围 IfxSrc_Tos_cpu0 IfxSrc_Tos_cpu1 IfxSrc_Tos_cpu2 IfxSrc_Tos_cpu3 IfxSrc_Tos_dma
//              不可设置为其他值

// 如果 INT_SERVICE 设置为 IfxSrc_Tos_dma 的话，ISR_PRIORITY 的可设置范围则是 0-127。

//================================================PIT 中断参数相关定义===============================================
#define CCU6_0_CH0_INT_SERVICE  IfxSrc_Tos_cpu0     // 定义CCU6_0 PIT通道0中断服务类型，即中断是由谁响应处理
#define CCU6_0_CH0_ISR_PRIORITY 30                  // 定义CCU6_0 PIT通道0中断优先级 优先级范围1-255 越大优先级越高
...
#define EXTI_CH0_CH4_INT_SERVICE IfxSrc_Tos_cpu0    // 定义ERU通道0和通道4中断服务类型
#define EXTI_CH0_CH4_INT_PRIO   255                 // 定义ERU通道0和通道4中断优先级

// 通道1与通道5是公用一个中断函数 在中断内部通过标志位 判断是谁触发的中断
#define EXTI_CH1_CH5_INT_SERVICE IfxSrc_Tos_dma     // 定义ERU通道1和通道5中断服务类型
#define EXTI_CH1_CH5_INT_PRIO   7                   // 定义ERU通道1和通道5中断优先级

// 通道2与通道6是公用一个中断函数 在中断内部通过标志位 判断是谁触发的中断
#define EXTI_CH2_CH6_INT_SERVICE IfxSrc_Tos_dma     // 定义ERU通道2和通道6中断服务类型
#define EXTI_CH2_CH6_INT_PRIO   6                   // 定义ERU通道2和通道6中断优先级 可设置范围为0-127(DMA响应)

// 通道3与通道7是公用一个中断函数 在中断内部通过标志位 判断是谁触发的中断
#define EXTI_CH3_CH7_INT_SERVICE IfxSrc_Tos_cpu0    // 定义ERU通道3和通道7中断服务类型
#define EXTI_CH3_CH7_INT_PRIO   254                 // 定义ERU通道3和通道7中断优先级


//===================================================DMA 中断参数相关定义===============================================
#define DMA_INT_SERVICE_1       IfxSrc_Tos_cpu0     // ERU触发DMA中断服务类型
#define DMA_INT_PRIO_1          0                   // ERU触发DMA中断优先级

//===================================================DMA 中断参数相关定义===============================================
#define DMA_INT_SERVICE_2       IfxSrc_Tos_cpu0     // ERU触发DMA中断服务类型
#define DMA_INT_PRIO_2          0                   // ERU触发DMA中断优先级


//===================================================串口中断参数相关定义===============================================
#define UART0_INT_SERVICE       IfxSrc_Tos_cpu0     // 定义串口0中断服务类型
#define UART0_TX_INT_PRIO       11                  // 定义串口0发送中断优先级
#define UART0_RX_INT_PRIO       10                  // 定义串口0接收中断优先级
#define UART0_ER_INT_PRIO       12                  // 定义串口0错误中断优先级
